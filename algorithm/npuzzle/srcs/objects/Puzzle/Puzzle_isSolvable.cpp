...
/*****
** Check if 2 Piece from Start Array are in an inverted order compared to Goal Array
**
...
*****/
static bool is_inverted(const int *goal, const int& nbr1, const int& nbr2, const int& size)
{
    int     nbr1_pos(0);
    int     nbr2_pos(0);

    for (int i = 0; i < size; ++i)  // Find position of nbr1 in Goal Array
        if (goal[i] == nbr1)
        {
            nbr1_pos = i;
            break ;
        }
    for (int i = 0; i < size; ++i)  // Find position of nbr2 in Goal Array
        if (goal[i] == nbr2)
        {
            nbr2_pos = i;
            break ;
...
/*****
** Get Inversion number from Start Array and Goal Array
**
...
** @return              : True if pieces are inverted, else False
*****/
static int  inversion_number(const int *start, const int *goal, const int& size)
{
    int     inv_nbr(0);
    ...
    for (int i = 0; i < size; ++i)      // For Each Piece (Except 0 Value), check if following ones are in inverted order
        if (start[i] != 0)
            for (int j = i + 1; j < size; ++j)
                if (start[j] != 0 && is_inverted(goal, start[i], start[j], size))
                    ++inv_nbr;
...
bool    Puzzle::isSolvable()
{
    ...
    {
        if (inversion_number(m_start->getArray(), m_goal->getArray(), m_infos.size) % 2 == 0)   // If Inversion number is even, Puzzle is Solvable
            return (true);
    }
    else if ((m_goal->getZeroPosition() / m_infos.length) % 2 == 0)     // Else if length of Puzzle even and Row of Zero position even (beginning from 0)
    {
        ...
        if ((zero_row_from_botom(m_start->getZeroPosition(), m_infos.length) % 2 == 0 && inversion_number(m_start->getArray(), m_goal->getArray(), m_infos.size) % 2 == 0)
            || (zero_row_from_botom(m_start->getZeroPosition(), m_infos.length) % 2 == 1 && inversion_number(m_start->getArray(), m_goal->getArray(), m_infos.size) % 2 == 1))
            return (true);
    }
    else
    {
        ...
        if ((zero_row_from_botom(m_start->getZeroPosition(), m_infos.length) % 2 == 0 && inversion_number(m_start->getArray(), m_goal->getArray(), m_infos.size) % 2 == 1)
            || (zero_row_from_botom(m_start->getZeroPosition(), m_infos.length) % 2 == 1 && inversion_number(m_start->getArray(), m_goal->getArray(), m_infos.size) % 2 == 0))
            return (true);
    }
